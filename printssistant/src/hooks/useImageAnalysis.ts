...
import { type DPIThreshold } from "../data/dpiGuidelines";
...
export function useImageAnalysis(): ImageAnalysisState {
  ...
  const [overallStatus, setOverallStatus] =
    useState<ImageAnalysisState["overallStatus"]>(null);
  ...
  const analyze = useCallback(
    async (_targetWidthIn: number, _targetHeightIn: number) => {
      setLoading(true);
      setError(null);
      setResults([]);
      ...
      try {
        ...
        setError(
          intl.formatMessage({
            defaultMessage:
              "To analyze image DPI, select images in your design and ensure they are high resolution for print. For best results, use images with at least 300 DPI at print size.",
            description: "Image analysis guidance",
          }),
        );
